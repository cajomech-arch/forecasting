...
temperature for New York City, giving us a comprehensive scenario of the future temperature
...
Applying the Box Jenkins methodology, we attempted to arrive using an accurate predictive model 
,which in our case was the SARIMA model. From identifying trend and seasonality, making the data 
stationary,
then applying the estimation techniques, and finally forecasting, we can see that the model
we chose based on our initial diagnostic of the ACF and PACF functions, SARIMA(1,1,1)(0,1,1)[12],
seems to give us an accurate fit to our forecasting goals. As we can see in our forecasted plot,
there is at the 95% significant level, that the data remains consistent to its historic values
and pattern. 
